...
- crossover(TSPChromosome that)
  - This method is used to generate new chromosome objects, The key code is shown as follows:
  ```
...
- mutation()
  - This method is used to mutate the chromosome, the key code is shown as follows:
  ```
...
- isValidateChromosome()
  - This method is used to debug and test to guarantee all of the chromosome lists are validated.
- compareTo(TSPChromosome that)
  - We override the compareTo method in order to sort the chromosome in generation algorithm
...
- initDataFromTxtFile(String filename)
  - This is used to initialize the whole generation algorithm because all the data are got from an external .txt file
- calFitnessList()
...
- select()
  ...
  - The best entity of each generation will keep alive without any change to keep the best genotype of an old generation
- evolution()
  - This method is used to crossover and mutation of the new generation's genotype
  - In our algorithm, the possibility of crossover is much higher than the possibility of mutation, this is an experiment conclusion
- bestEntity()
...
- java.util.concurrent.CompletableFuture
  ...
  ```
- We divided the population into two subpopulations and generate each sub-population in thread separately, and after we complete generate all the sub-populations, we will merge the best entities of each sub-population together as the new generation and copy the result to the new chromosome list.
...
- We created a user interface which users can upload a file and run the generation algorithm.
- At first, the user interface is as follows and the user can upload a data file to it, the screenshot is as follows:
  - ![InitialUI](screenshot/InitialUI.png)
- After uploading a data file, the user can see the parameters including shortest distance, the best genotype, and phenotype from the UI, the screenshot is as following (the conclusion is obtained by data.txt)
...
## Conclusion
In sum, we created a genetic algorithm to solve the travelling salesman problem. In the evidence of user interface output, we can see that the algorithm will find the shortest path in travelling salesman problem whatever the input is. While the algorithm didn't work so well in the earlier versions. By changing some parameters, we found many parameters have influences to the algorithm:
...
- Crossover possibility: We found crossover is an amazing way to envolve better entities. That is because, in every generation, we choose good entities to generate new population, so the crossover will combine good genes in different entities together as a new entity. This is pretty efficient. So we set the crossover possibility very close to "1" to make our algorithm efficiently.
